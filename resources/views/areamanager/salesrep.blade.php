...
<div class="modal fade add_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="add-form" method="post" action="<?php echo URL::to('areamanager/sales_rep_add')?>" enctype="multipart/form-data">
                <div class="modal-header">
...
<div class="modal fade status_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form action="<?php echo URL::to('areamanager/sales_rep_status')?>">
                <div class="modal-header">
...
<div class="modal fade sim_allocation_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <form method="post" action="<?php echo URL::to('areamanager/add_stock')?>">
                <div class="modal-header">
                ...
                ?>
                  ...
                  if(count($userlistarea)){
                    foreach ($userlistarea as $usersingle) {
                        $user = DB::table('users')->where('user_id', $usersingle)->first();
                        ...
                        <tr>
                          ...
                          <a href="'.URL::to('areamanager/view_stock/'.base64_encode($user->user_id)).'" data-toggle="tooltip" data-placement="top" data-original-title="View Stock of Month"><i class="far fa-calendar-alt"></i></a> &nbsp;&nbsp;&nbsp;

                          <a href="'.URL::to('areamanager/time_sheet/'.base64_encode($user->user_id)).'" data-toggle="tooltip" data-placement="top" data-original-title="Time Sheet"><i class="fas fa-clock"></i></a>    
...
if($(e.target).hasClass("active_class")){
    ...
    setTimeout(function() {
      $.ajax({
          url:"<?php echo URL::to('areamanager/sales_rep_details')?>",
          dataType:'json',
...
if($(e.target).hasClass("total_sim")){
  ...
  setTimeout(function() {
    ...
    var value = $(e.target).attr("data-element");
    $.ajax({
        url:"<?php echo URL::to('areamanager/salesrep_sim_total')?>",
        dataType:'json',
        data:{id:value},
        type:"post",
        success: function(result)
...
if($(e.target).hasClass("active_sim")){
  ...
  setTimeout(function() {
    ...
    var value = $(e.target).attr("data-element");
    $.ajax({
        url:"<?php echo URL::to('areamanager/salesrep_sim_active')?>",
        dataType:'json',
        data:{id:value, type:2},
        type:"post",
        success: function(result)
...
if($(e.target).hasClass("inactive_sim")){
  ...
  var value = $(e.target).attr("data-element");
  $.ajax({
      url:"<?php echo URL::to('areamanager/salesrep_sim_inactive')?>",
      dataType:'json',
      data:{id:value, type:2},
      type:"post",
      success: function(result)
...
if($(e.target).hasClass("refresh_icon")){
  ...
  var value = $(e.target).attr("data-element");
  $.ajax({
      url:"<?php echo URL::to('areamanager/refresh_salesrep')?>",
      dataType:'json',
      data:{id:value},
      type:"post",
      success: function(result)
...
if($(e.target).hasClass("filter_area")){
  ...
  setTimeout(function() { 
    ...
    $.ajax({
      url:"<?php echo URL::to('areamanager/filter_for_salesrep')?>",
      dataType:'json',
      data:{id:id,type:1},
      type:"post",
      success: function(result)
...
$('#add-form').validate({
    rules: {    
      ...
      email_id: {required: true, remote:"<?php echo URL::to('areamanager/users_email_check')?>"},
...
$('#edit-form').validate({
    rules: {
      ...
      postcode:{required: true},
      mobile:{required: true},
      email_id : {required: true, remote: { url:"<?php echo URL::to('areamanager/users_email_check')?>", 
