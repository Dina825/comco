		          	...
		          	if(count($stocklist)){
		          		foreach ($stocklist as $stock) {
		          			...
		          			if($stock->submitted == 0){
		          				...
		          				$send = 'Send by '.$area_manager->firstname.' '.$area_manager->surname;

		          			}
		          			$output_stock.='
		          			<tr>
		          				<td>'.$i.'</td>
		          				<td><a href="'.URL::to('areamanager/view_stock_details/'.base64_encode($stock->stock_id)).'">'.date('d-M-Y', strtotime($stock->stock_date)).' '.$send.'</a> </td>
