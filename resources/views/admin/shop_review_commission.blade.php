...
<div class="modal fade note_model" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg ">
    <div class="modal-content">
      <form method="post" action="<?php echo URL::to('admin/note_update')?>"  enctype="multipart/form-data" id="note-form">
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-12">                    
      						<div class="form-group">
      						    ...
      						    <select class="form-control add_input" required name="salesrep">
      						...
      						<div class="form-group">
      						    ...
      						    <input type="number" class="form-control amount_class add_input" placeholder="Enter Amount" name="amount">
      						    <input type="hidden" value="<?php echo base64_encode($shop_details->shop_id)?>" name="shop_id">
      						</div>
      						<div class="form-group">
      						    <label>Cheque Number</label>
      						    <input type="text" class="form-control cheque_class add_input" placeholder="Cheque Number" name="number">      						    
      						</div>
      				  </div>                                      
            </div>                
      </div>
...
<div class="modal fade received_model" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form method="post" action="<?php echo URL::to('admin/cheque_received')?>">
                <div class="modal-header">
                    <h5 class="modal-title received_title" id="exampleModalLabel"></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="sub_title3 received_content" style="line-height: 25px;"></div>
	...
	<div class="row">
		...
		<div class="col-lg-6 col-sm-6 col-6 margin_top_30">
			<h6>Commission</h6>			
		</div>
		<div class="col-lg-6 col-sm-6 col-6 text-right" style="display: block;">
	      <div class="dropdown">
            ...
            <?php
            $get_shop_dates2 = DB::table('sim_processed')->where('shop_id',$_GET['shop_id'])->orderBy('month_year','desc')->groupBy('month_year')->get();
            if(count($get_shop_dates2)){
              $release_button='display:block';
            }
            else{
              $release_button ='display:none';
            }
            ?>
	          <a href="javascript:" style="<?php echo $release_button?>"  data-toggle="modal" data-target=".release_model" class="dropdown-item release_cheque">Release Cheque</a>
	        </div>
	      </div>
	    </div>
		...
		<div class="col-lg-12">
		        ...
		        <table class="own_table">
		        	...
		        	<tbody>
		        		...
		        		<tr>
		        			...
		        			<td></td>
		        			<td></td>
		        			<td align="right"><b>&#163; <?php echo number_format_invoice($commission_total)?></b></td>
		        			<td align="right"><b>&#163;  <?php echo number_format_invoice($bonus_total) ?></b></td>
		        			<td></td>
		        		</tr>
		        		<?php
		        		}
		  ...
		  <div class="col-lg-12 margin_top_30" style="<?php echo $display_paid?>">
		        ...
		        <table class="own_table">
		        	<thead>
		        		<tr>
		        			...
		        			<th style="width: 110px;">Sales REP</th>
		        		...
		        		if(count($paid_details)){
		        			foreach ($paid_details as $paid) {
		        				...
		        				elseif($paid->status == 2){
		        					$color = "#f00";
		        					$minus = '-';
		        					$status = '<a href="javascript:" class="received_class" data-element="'.base64_encode($paid->paid_id).'" style="color:#f00">Cheque Return</a>';
		        				}
		        				...
		        				$salesrep_details = DB::table('sales_rep')->where('user_id', $paid->salesrep)->first();


		        				$paid_output.='
		        			<tr style="color:'.$color.'">
		        				<td>'.$i.'</td>
		        				<td style="width:100px">'.date('d-M-Y', strtotime($paid->date)).'</td>
		        				<td>'.$salesrep_details->firstname.' '.$salesrep_details->surname.'</td>
		        				<td>'.$paid->description.'</td>
		        				<td>'.$paid->cheque_number.'</td>
		        				<td style="text-align: right;">'.$minus.' &#163; '.number_format_invoice($paid->commission).'</td>
		        				<td style="text-align: right;">'.$symbol.' '.number_format_invoice($bonus).'</td>
		        			...
		        			<tr>
		        				...
		        				<td></td>
		        				<td></td>
		        				<td style="text-align: right;"><b>&#163; '.number_format_invoice($total_paid_commission).'</b></td>
		        				<td style="text-align: right;">'.$paid_symbol.''.number_format_invoice($total_paid_bonus).'</td>
		        				<td></td>
		        				<td></td>
		        			</tr>';

		        			$peding_details = '
		        			<table class="own_table" style="margin-top:10px;">
		        			<tr>
		        				<td></td>
		        				<td></td>
		        				<td></td>
		        				<td style="text-align: right;"><b>Total Commission</b></td>
		        				<td style="text-align: right;"><b>&#163; '.number_format_invoice($commission_total).'<b></td>
		        				<td style="text-align: right;"><b>Total Bonus</b></td>
		        				<td style="text-align: right;"><b>&#163; '.number_format_invoice($bonus_total).'</b></td>
		        			</tr>
		        			<tr style="color:#f00">
		        				<td></td>
		        				<td></td>
		        				...
		        				<td style="text-align: right;"><b>'.$paid_minus.' '.$paid_symbol.''.number_format_invoice($total_paid_bonus).'</b></td>
		        			</tr>
		        			<tr>
		        				<td></td>
		        				<td></td>
		        				...
		        				<td style="text-align: right;"><b>&#163; '.number_format_invoice($pending_commission).'<b><input type="hidden" class="pending_commission_class" value="'.$pending_commission.'" > </td>
		        				<td style="text-align: right;"><b>Pending Bonus</b></td>
		        				<td style="text-align: right;"><b>&#163; '.number_format_invoice($pending_bonus).'</b></td>
		        			</tr>
		        			</table>
		        			';
	        ...
	        <div class="row margin_top_30" style="display: <?php if(count($paid_details)){echo 'block';}else{ echo 'none';}?>" >
				<div class="col-lg-12 col-md-12 col-12 col-sm-12">
					<h6>Details of Commission</h6>
				</div>
				<div class="col-lg-12 col-md-12 col-12 col-sm-12">
					<div class="table-responsive">
  ...
  setTimeout(function() {
    ...
    $.ajax({
        ...
        success: function(result)
        {
          ...
          $(".cheque_class").val(result['cheque_number']);
...
if($(e.target).hasClass("received_class")){
  $(".loading_css").addClass("loading");
  setTimeout(function() {
    var value = $(e.target).attr("data-element");
    $.ajax({
        ...
        success: function(result)
        {
          $(".received_title").html(result['title']);
          $(".id_filed").val(result['id']);
          $(".received_content").html(result['content']);          
          $(".received_model").modal('show');
          $(".loading_css").removeClass("loading");
        }
      })
    },500);
...
$( document ).ready(function() {
   var balance = $(".pending_commission_class").val();
   $(".amount_class").attr({"max" : balance});
});
