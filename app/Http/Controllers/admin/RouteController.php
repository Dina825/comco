...
class RouteController extends Controller {
	...
	*/
	...
	public function route()
	{
		...
		return view('admin/route', array('title' => 'Manage Route', 'arealist' => $area, 'userlist' => $user, 'routelist' => $route));
	}
	...
	public function routeupdate(){
		...
		if(count($sales_checkbox) == 1){
			$route_shop_list = DB::table('shop')->select('route', 'shop_id', 'sales_rep')->where('route',$id)->get();

			if(count($sales_checkbox)){
				foreach ($sales_checkbox as $salesrep) {

					if(count($route_shop_list)){
						foreach ($route_shop_list as $route_shop) {
							$datasales['sales_rep'] = $salesrep;
							DB::table('shop')->where('shop_id', $route_shop->shop_id)->update($datasales);
						}
					}
