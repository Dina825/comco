				...
				foreach ($objPHPExcel->getWorksheetIterator() as $keysheet => $worksheet) {
					if($keysheet == 0)
					{
							...
							for ($row = 2; $row <= $height; ++ $row) {
								...
								if(count($check_ssn))
								{
									if($activity_date != "")
									{
										...
										$excel_date = 25569 + ($unix_date / 86400);
										$unix_date = ($excel_date - 25569) * 86400;
										$activity_date_excel = gmdate("Y-m-d", $unix_date);
										$data['activity_date'] = $activity_date_excel;
						...
						if($height >= $highestRow)
						{
							...
							return redirect('admin/simcards?filename='.$name.'&height='.$height.'&round=2&highestrow='.$highestRow.'&import_type_new=2');
						}
					}
				}
			}
	...
	public function import_activation_sim_one()
	{
		...
		foreach ($objPHPExcel->getWorksheetIterator() as $keysheet => $worksheet) {
			...
			if($height >= $highestRow)
			{
				...
				return redirect('admin/simcards?filename='.$name.'&height='.$height.'&round='.$nextround.'&highestrow='.$highestRow.'&import_type_new=2');
			}
		}
	}
